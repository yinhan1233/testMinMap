 ...
 * Tah 温湿度
 */
class Tah extends THING.Marker {
    constructor(app) {
        super(app);
        this.racks = [];
        this.isTah = true;
    ...
    createUI(val) {
        ...
        var panel = new THING.widget.Panel({
            ...
            width: "260px",
            titleText: '温湿度详情',
            // localPosition: [self.position[0], self.position[1] + 2, self.position[2]],
            closeIcon: true,
            retractable: true,
            opacity: 0.8,
            visible: true
        ...
        this.panel = panel;
        this.userData.meteValue = this.userData.meteValue + (this.userData.meteValue ? '℃' : '');
        this.userData.meteValue1 = this.userData.meteValue1 + (this.userData.meteValue1 ? '%RH' : '');
        // panel.addString(this.userData, 'meteName').caption('点位');
        // panel.addString(this.userData, 'meteValue').caption('温度');
        // this.userData.meteValue1 != '' && panel.addString(this.userData, 'meteValue1').caption('湿度');
        let base = {
            "点位": this.userData.meteName || '',
            "温度": this.userData.meteValue || '',
        }
        this.userData.meteValue1 != '' && (base['湿度'] = this.userData.meteValue1 || '')
        let alarm = {
    ...
    createTahUI() {
        ...
        this.panel = panel;
        let base = {
            "温度点位": this.userData.meteName || '',
            // "温度": this.userData.meteValue || '',
        }
        panel.addString(base, '温度点位')
        // 创建obj ui (跟随物体用)
        var ui = app.create({
            type: 'UI',
            parent: this,
    ...
    // 创建长久显示的panel
    showTahUI(boolValue) {
        // this.permanentPanel && (this.permanentPanel.visible = boolValue);
        this.panel.visible = boolValue
    }
}
THING.factory.registerClass('Tah', Tah);
Tah.current = null;
Tah.open = false;
